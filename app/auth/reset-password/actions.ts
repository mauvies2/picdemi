...
export async function updatePasswordAction(formData: FormData) {
  ...
  if (password.length < 8) {
    return redirect(
      `/auth/reset-password?message=Password must be at least 8 characters`,
    );
  }
  ...
  if (!/[0-9]/.test(password)) {
    return redirect(
      `/auth/reset-password?message=Password must contain at least one number`,
    );
  }
  ...
  if (!user) {
    return redirect(
      `/login?message=Session expired. Please request a new password reset link.`,
    );
  }
  ...
  if (error) {
    console.error(error);
    return redirect(
      `/auth/reset-password?message=Could not update password. Please try again.`,
    );
  }
