...
export default function NewEventForm() {
  ...
  const eventSummary = useMemo(() => {
    ...
    return [
      ...
      ...(source.state
        ? [{ label: "State / Province", value: source.state }]
        : []),
    ...
    <div className="p-4">
      <div className="mx-auto w-full max-w-full space-y-6">
        ...
        <div className="grid min-w-0 gap-10 lg:grid-cols-2 lg:items-start">
          {/* Dropzone section - appears first on mobile, left side on desktop */}
          <div className="order-1 min-w-0 w-full space-y-2 lg:sticky lg:top-4">
            <Dropzone
            ...
            />
            {files.length > 0 && (
              <div className="w-full max-w-full overflow-hidden rounded-xl border border-dashed border-muted-foreground/30 p-4">
                <ul className="grid gap-2">
                  {files.map((file) => (
                    <li
                      key={`${file.name}-${file.lastModified}`}
                      className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-transparent px-3 py-2 transition-colors hover:border-muted-foreground/30"
                    >
          ...
          {/* Form section - appears second on mobile, right side on desktop */}
          <form
            className="order-2 min-w-0 w-full flex flex-col gap-6"
            onSubmit={(event) => {
