...
export function AIMatchingModal({ open, onOpenChange }: AIMatchingModalProps) {
  ...
  const handleSearch = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        ...
        const fileToUse =
          selfieFile ?? new File([], "placeholder.jpg", { type: "image/jpeg" });
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to run AI search. Please try again.",
        );
        setStep("filters");
      }
    });
  ...
  const handleAddToLibrary = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        const { added } = await addMatchedPhotosToLibrary(
          Array.from(selectedPhotoIds),
        );
        toast.success(
          `Added ${added} photo${added === 1 ? "" : "s"} to your library`,
        );
        onOpenChange(false);
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Failed to add photos to library",
        );
      }
    });
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {availability.description}. Please upgrade your plan for more
                  searches.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1.5 mb-4">
                  For best results, use a clear photo with good lighting. Avoid
                  photos with goggles, helmets, or water sports gear that
                  obscure your face.
                </p>
                {selfiePreview ? (
