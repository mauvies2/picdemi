...
export async function getAISearchUsageCount(
  ...
  if (error) {
    ...
    if (queryError && queryError.code !== 'PGRST116') {
      throw new Error(`Failed to get AI search usage: ${getErrorMessage(queryError)}`);
    }
...
export async function incrementAISearchUsage(
  ...
  if (error) {
    ...
    if (selectError && selectError.code !== 'PGRST116') {
      throw new Error(`Failed to get AI search usage: ${getErrorMessage(selectError)}`);
    }
    ...
    if (existingUsage) {
      ...
      if (updateError) {
        throw new Error(`Failed to increment AI search usage: ${getErrorMessage(updateError)}`);
      }
    ...
    if (insertError) {
      throw new Error(`Failed to create AI search usage: ${getErrorMessage(insertError)}`);
    }
